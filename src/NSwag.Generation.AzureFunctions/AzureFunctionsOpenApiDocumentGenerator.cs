...
namespace NSwag.Generation.AzureFunctions
{
    ...
    /// </summary>
    public class AzureFunctionsOpenApiDocumentGenerator
    {
        ...
        /// </summary>
        public AzureFunctionsOpenApiDocumentGeneratorSettings Settings { get; }

        /// <summary>Initializes a new instance of the <see cref="AzureFunctionsOpenApiDocumentGenerator" /> class.</summary>
        /// <param name="settings">The settings</param>
        public AzureFunctionsOpenApiDocumentGenerator(AzureFunctionsOpenApiDocumentGeneratorSettings settings)
        {
            Settings = settings;
        }
        ...
        public async Task<OpenApiDocument> GenerateForAzureFunctionClassesAsync(IReadOnlyList<Type> azureFunctionClassTypes,
                                                                                IReadOnlyList<string> functionNames)
        {
            ...
            foreach (var azureFunctionClassType in azureFunctionClassTypes)
            {
                ...
                var isIncluded = await GenerateForAzureFunctionClassAsync(document,
                    generator, schemaResolver, azureFunctionClassType, functionNames);
                if (isIncluded)
        ...
        private async Task<bool> GenerateForAzureFunctionClassAsync(OpenApiDocument document, OpenApiDocumentGenerator swaggerGenerator,
                                                                    OpenApiSchemaResolver schemaResolver, Type azureFunctionClassType,
                                                                    IReadOnlyList<string> functionNames)
        {
            var operations = new List<Tuple<OpenApiOperationDescription, MethodInfo>>();

            foreach (var method in GetActionMethods(azureFunctionClassType, functionNames))
            {
                    ...
                    foreach (var httpMethod in httpMethods)
                    {
                        var operationDescription = new OpenApiOperationDescription
                        {
                            ...
                            Operation = new OpenApiOperation
                            {
                                ...
                                OperationId = GetOperationId(document, azureFunctionClassType.Name, method)
                            }
                        };
            ...
            return await AddOperationDescriptionsToDocumentAsync(document, azureFunctionClassType, operations,
        ...
        /// <returns></returns>
        private static IReadOnlyList<MethodInfo> GetActionMethods(Type azureFunctionStaticClassType, IReadOnlyList<string> functionNames)
        {
        ...
        /// <returns></returns>
        private IReadOnlyList<string> GetHttpPaths(MethodInfo method)
        {
        ...
        /// <param name="method"></param>
        /// <returns></returns>
        private IReadOnlyList<string> GetSupportedHttpMethods(MethodInfo method)
        {
